 ...
 * @private
 */
CodeEditor.TABS_ = ['blocks', 'python', 'xml'];
...
CodeEditor.tabClick = function(clickedName) {
  ...
  if (document.getElementById('tab1_xml').className == 'tabon') {
    var xmlTextarea = document.getElementById('content1_xml');
    ...
    if (xmlDom) {
      ...
      Blockly.Xml.domToWorkspace(xmlDom, CodeEditor.workspace);
    }
  }
...
CodeEditor.renderContent = function() {
  ...
  // Initialize the pane.
  if (content.id == 'content1_xml') {
    var xmlTextarea = document.getElementById('content1_xml');
    var xmlDom = Blockly.Xml.workspaceToDom(CodeEditor.workspace);
    var xmlText = Blockly.Xml.domToPrettyText(xmlDom);
    xmlTextarea.value = xmlText;
    xmlTextarea.focus();
  } else if (content.id == 'content1_python') {
